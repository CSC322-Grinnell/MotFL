	...
	def self.search(search)
		if search != nil
			sql = "SELECT resource_metadata.id, Title, GROUP_CONCAT(authors.author_name, ', ') as authors, publish_date, Abstract, Link 
					...
					HAVING Tag_Title LIKE '%#{search.downcase}%' OR
			   				Title LIKE '%#{search.downcase}%' OR
			   				Abstract LIKE '%#{search.downcase}%'OR
								authors LIKE '%#{search.downcase}%' OR
								Link LIKE '%#{search.downcase}%'"
